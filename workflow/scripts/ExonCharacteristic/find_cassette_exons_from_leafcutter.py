#!/usr/bin/env python3
"""
Cassette Exon Identification from LeafCutter Data

Identifies potential cassette exons from LeafCutter junction clusters.
Does not require a skipping junction to be present.

Usage:
    python find_cassette_exons.py \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Identify cassette exons from LeafCutter junction data",
    ...
    parser.add_argument(
        "--leafcutter-counts", "-c",
        required=True,
        help="Path to LeafCutter junction counts file (space-delimited, junction IDs as row names)"
    )
    parser.add_argument(
...
def load_and_parse_counts(counts_path: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Load LeafCutter counts and parse junction IDs to extract cluster info.
    
    Junction ID format: chr:start:stop:clu_N_strand
    Example: GL000194.1:53862:54678:clu_1_-
    
    Note: LeafCutter uses stop+1 in the ID, so we subtract 1 to get actual stop.
    """
    ...
    for jid in junction_ids:
        ...
        # Correct stop coordinate (LeafCutter uses stop+1)
        stop = stop_plus1 - 1
        
        # Create Intron_coord for joining with annotation
...
def find_cassette_exons(
    ...
    # Construct expected skipping junction ID
    # Format: chr:start:stop+1:clu_N_strand
    cassettes["skip_junction_expected"] = (
        cassettes["chrom"] + ":" +
        cassettes["skip_start"].astype(str) + ":" +
        (cassettes["skip_stop"] + 1).astype(str) + ":" +
        "clu_" + cassettes["clu_num"] + "_" + cassettes["Strand"]
    )
    
    # Look up reads for skipping junction
    cassettes["reads_skip"] = cassettes["skip_junction_expected"].map(
        lambda x: junction_reads.get(x, 0)
    )
...
def export_results(cassettes: pd.DataFrame, output_dir: str) -> pd.DataFrame:
    ...
    cassette_table = cassettes[[
        ...
        "skip_junction_expected", "reads_upstream", "reads_downstream",

...
Finds CDS features at the donor and acceptor splice sites of productive junctions
that are associated with poison donor/acceptor events. Annotates which CDS is
shared (unchanged) vs. alternative between productive and poison junctions.
...
def filter_productive_junctions(productive_junctions_str, poison_start, poison_end,
                                junction_type, filter_mode='closest'):
    """
    Filter productive junctions to keep only the most relevant ones.

    Args:
        productive_junctions_str: Comma-separated productive junction coordinates
        poison_start: Poison junction start coordinate
        poison_end: Poison junction end coordinate
        junction_type: 'poisonDonor' or 'poisonAcceptor'
        filter_mode: 'closest', 'threshold', or 'none'

    Returns:
        Filtered comma-separated productive junction string
    """
    if pd.isna(productive_junctions_str) or productive_junctions_str == '':
        return ''

    productive_junctions = str(productive_junctions_str).split(',')

    if filter_mode == 'none' or len(productive_junctions) <= 1:
        return productive_junctions_str
    ...
    for prod_junc in productive_junctions:
        ...
        if junction_type == 'poisonAcceptor':
            # Alternative site is acceptor - compare acceptor positions
            distance = abs(prod_end - poison_end)
        elif junction_type == 'poisonDonor':
            # Alternative site is donor - compare donor positions
...
def process_poison_donor_acceptor(pd_file, cds_index, tolerance=0, chromosome=None, filter_mode='closest'):
    """
    Process poison donor/acceptor junctions and find flanking CDS features.

    Args:
        pd_file: Poison donor/acceptor BED file
        cds_index: Dict of CDS features
        tolerance: Coordinate matching tolerance
        chromosome: Optional chromosome filter
        filter_mode: Productive junction filtering mode ('closest', 'threshold', or 'none')

    Returns:
        (pd_with_flanks, flanking_cds_list)
    """

    print(f"\nReading poison donor/acceptor junctions: {pd_file}")
    ...
    for idx, row in pd_df.iterrows():
        ...
        productive_junctions_str = row['productive_junctions']

        # Skip if no productive junctions (NaN or empty)
        if pd.isna(productive_junctions_str) or productive_junctions_str == '':
            not_found_count += 1
            continue

        # Filter productive junctions to keep only relevant ones
        productive_junctions_filtered = filter_productive_junctions(
            productive_junctions_str, start, end, junction_type, filter_mode
        )

        if not productive_junctions_filtered:
            not_found_count += 1
            continue

        # Parse productive junctions (may be comma-separated)
        productive_junctions = str(productive_junctions_filtered).split(',')

        all_donor_cds = []
        all_acceptor_cds = []

        # Process each productive junction
        for prod_junc in productive_junctions:
            prod_junc = prod_junc.strip()
        ...
        if junction_type == 'poisonDonor':
            shared_type = 'acceptor_CDS'
            alternative_type = 'donor_CDS'
        elif junction_type == 'poisonAcceptor':
        ...
        # Deduplicate productive junctions as well
        productive_junctions_unique = list(dict.fromkeys([pj.strip() for pj in productive_junctions]))
        productive_junctions_str_unique = ",".join(productive_junctions_unique)

        # Poison junction identifier
        pj_id = f"{chrom}:{start}-{end}_{gene_name}"

        # Calculate poison event exon regions
        ...
        if junction_type == 'poisonAcceptor':
            ...
            if strand == '+':
                # Poison acceptor is at end position
                poison_acceptor_pos = end
                for prod_junc in productive_junctions_unique:
                    _, prod_start, prod_end = parse_coordinate(prod_junc.strip())
                    # Splice region: from productive acceptor to poison acceptor
                    if prod_end < poison_acceptor_pos:
                        splice_regions.append(f"{chrom}:{prod_end}-{poison_acceptor_pos}")
                    elif prod_end > poison_acceptor_pos:
                        splice_regions.append(f"{chrom}:{poison_acceptor_pos}-{prod_end}")
                    else:
                        splice_regions.append(f"{chrom}:{prod_end}-{poison_acceptor_pos}")

                # Full exon: from poison acceptor to acceptor CDS end
                if acceptor_cds_str != "NA":
                    first_cds = acceptor_cds_str.split(',')[0].strip()
                    _, cds_start, cds_end = parse_coordinate(first_cds)
                    poison_full_exon = f"{chrom}:{poison_acceptor_pos}-{cds_end}"
                else:
                    poison_full_exon = "NA"

            else:  # strand == '-'
                # Poison acceptor is at start position (biologically)
                poison_acceptor_pos = start
                for prod_junc in productive_junctions_unique:
                    _, prod_start, prod_end = parse_coordinate(prod_junc.strip())
                    # Splice region: from productive acceptor to poison acceptor
                    if prod_start < poison_acceptor_pos:
                        splice_regions.append(f"{chrom}:{prod_start}-{poison_acceptor_pos}")
                    elif prod_start > poison_acceptor_pos:
                        splice_regions.append(f"{chrom}:{poison_acceptor_pos}-{prod_start}")
                    else:
                        splice_regions.append(f"{chrom}:{prod_start}-{poison_acceptor_pos}")

                # Full exon: from acceptor CDS start to poison acceptor
                if acceptor_cds_str != "NA":
                    first_cds = acceptor_cds_str.split(',')[0].strip()
                    _, cds_start, cds_end = parse_coordinate(first_cds)
                    poison_full_exon = f"{chrom}:{cds_start}-{poison_acceptor_pos}"
                else:
                    poison_full_exon = "NA"

            poison_splice_region = ",".join(splice_regions) if splice_regions else "NA"

        elif junction_type == 'poisonDonor':
            splice_regions = []

            if strand == '+':
                # Poison donor is at start position
                poison_donor_pos = start
                for prod_junc in productive_junctions_unique:
                    _, prod_start, prod_end = parse_coordinate(prod_junc.strip())
                    # Splice region: from poison donor to productive donor
                    if poison_donor_pos < prod_start:
                        splice_regions.append(f"{chrom}:{poison_donor_pos}-{prod_start}")
                    elif poison_donor_pos > prod_start:
                        splice_regions.append(f"{chrom}:{prod_start}-{poison_donor_pos}")
                    else:
                        splice_regions.append(f"{chrom}:{poison_donor_pos}-{prod_start}")

                # Full exon: from donor CDS start to poison donor
                if donor_cds_str != "NA":
                    first_cds = donor_cds_str.split(',')[0].strip()
                    _, cds_start, cds_end = parse_coordinate(first_cds)
                    poison_full_exon = f"{chrom}:{cds_start}-{poison_donor_pos}"
                else:
                    poison_full_exon = "NA"

            else:  # strand == '-'
                # Poison donor is at end position (biologically)
                poison_donor_pos = end
                for prod_junc in productive_junctions_unique:
                    _, prod_start, prod_end = parse_coordinate(prod_junc.strip())
                    # Splice region: from poison donor to productive donor
                    if poison_donor_pos < prod_end:
                        splice_regions.append(f"{chrom}:{poison_donor_pos}-{prod_end}")
                    elif poison_donor_pos > prod_end:
                        splice_regions.append(f"{chrom}:{prod_end}-{poison_donor_pos}")
                    else:
                        splice_regions.append(f"{chrom}:{poison_donor_pos}-{prod_end}")

                # Full exon: from poison donor to donor CDS end
                if donor_cds_str != "NA":
                    first_cds = donor_cds_str.split(',')[0].strip()
                    _, cds_start, cds_end = parse_coordinate(first_cds)
                    poison_full_exon = f"{chrom}:{poison_donor_pos}-{cds_end}"
                else:
                    poison_full_exon = "NA"
        ...
        results.append({
            ...
            'productive_junctions': productive_junctions_str_unique,
            'donor_CDS': donor_cds_str,
            'acceptor_CDS': acceptor_cds_str,
            'shared_CDS': shared_type,
            'alternative_CDS': alternative_type,
            'poison_event_splice_region': poison_splice_region,
            'poison_event_CDS_region': poison_full_exon
        })
        ...
        for cds in all_donor_cds:
            for prod_junc in productive_junctions:
                flanking_cds_features.append({
                    ...
                    'poisonJunction': pj_id,
                    'junction_type': junction_type,
                    'intron_coord': intron_coord,
                    'productive_junction': prod_junc.strip(),
                    'transcript_id': cds[3]
                })
...
def main():
    parser = argparse.ArgumentParser(
        description='Identify flanking CDS features for poison donor/acceptor junctions',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example:
  python flanking_CDS_poisonDonorAcceptor.py \\
      -g Reference.gtf \\
      -p poisonDonorAcceptor.bed \\
      -o output/poison_da_flanking.tsv \\
      -f output/flanking_cds.tsv \\
      --tolerance 0 \\
      --chromosome chr1

Flanking CDS identification:
  - For each productive junction associated with a poison donor/acceptor event
  ...
  - Poison Donor: acceptor is shared, donor is alternative
  - Poison Acceptor: donor is shared, acceptor is alternative
...
Output format:
  - All coordinates in 0-based BED format (start position is -1 from 1-based)
    ...
    parser.add_argument('-p', '--poison-bed', required=True,
                       help='Poison donor/acceptor BED file')
    ...
    parser.add_argument('--filter-mode', type=str, default='closest',
                       ...
                       help='Productive junction filtering mode (default: closest)')

    args = parser.parse_args()

    print("="*70)
    print("IDENTIFY FLANKING CDS FOR POISON DONOR/ACCEPTOR JUNCTIONS")
    print("="*70)
    print(f"GTF: {args.gtf}")
    print(f"Poison donor/acceptor BED: {args.poison_bed}")
    ...
    # Process poison donor/acceptor junctions
    pd_with_flanks, flanking_cds = process_poison_donor_acceptor(
        args.poison_bed, cds_index, tolerance=args.tolerance, chromosome=args.chromosome,
    ...
    pd_before = len(pd_with_flanks)
    flanking_before = len(flanking_cds)

    pd_with_flanks = pd_with_flanks.drop_duplicates()
    flanking_cds = flanking_cds.drop_duplicates()

    pd_removed = pd_before - len(pd_with_flanks)
    flanking_removed = flanking_before - len(flanking_cds)

    if pd_removed > 0:
        print(f"  Removed {pd_removed:,} duplicate junction rows")
    else:
    ...
    pd_with_flanks.to_csv(args.output, sep='\t', index=False)
    print(f"  Saved {len(pd_with_flanks):,} unique junctions to: {args.output}")

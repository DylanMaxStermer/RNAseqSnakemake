...
Integrated Poison Exon Detection Pipeline
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description="Integrated poison exon detection and analysis pipeline",
    ...
    parser.add_argument('-c', '--leafcutter-counts', required=True, 
                        help='Path to raw LeafCutter counts file (gzipped)')
...
def run_step1(args, pe_dir):
    ...
    if not filtered_df.empty:
        ...
        print("[Step 1] No poison exons remained after filtering", file=sys.stderr)
...
# this first one works with my old data 

# def parse_leafcutter_counts(input_file, output_file):
#     """Parses raw LeafCutter counts file into structured format."""
#     print(f"Parsing LeafCutter counts from: {input_file}")
...
#             original_string = row[0]  # Store the original string
...
# this works with my new data – there is something off with the cordinantes 

def parse_leafcutter_counts(input_file, output_file):
    """Parses raw LeafCutter counts file into structured format."""
    print(f"[Step 2] Parsing LeafCutter counts from: {input_file}", file=sys.stderr)
    ...
    with gzip.open(input_file, 'rt') as infile, \
        ...
        for line in infile:
            ...
            chr_name = intron_info[0]
            start = intron_info[1]
            stop = str(int(intron_info[2]) - 1)  # FIX: Subtract 1 from end coordinate
            cluster = f'clu_{intron_info[3].split("_")[1]}'
            strand = intron_info[3].split('_')[2]
            intron_coord = f'{chr_name}:{start}-{stop}'
...
def main():
    ...
    print("\n" + "="*80, file=sys.stderr)
    print("INTEGRATED POISON EXON DETECTION PIPELINE", file=sys.stderr)
    ...
    print(f"Leafcutter counts: {args.leafcutter_counts}", file=sys.stderr)
    ...
    try:
        ...
        print(f"  8. Parsed LeafCutter counts: {os.path.basename(args.output_prefix)}_parsed_counts.gz", file=sys.stderr)

...
rule MakeColored_Transcripts_bed:
    ...
    shell:
        """
        python workflow/scripts/leafcutter2/scripts/Reformat_gtf.py -i {input.gtf} -fa {input.fa} -bed12 Transcripts.colored.bed  -o Transcripts.gtf -infer_gene_type_approach B -translation_approach C -gene_name_attribute_name gene_id
...
rule Extract_introns:
    ...
    shell:
        """
        (python workflow/scripts/ExtractIntronsFromGtf.py --gtf {input.gtf} --reference {input.fa} --output /dev/stdout |  awk -F'\\t' -v OFS='\\t' '{{split($4, a, "|"); print $1, $2, $3, a[2]"|"a[3], $5, $6}}' | sort | uniq | bedtools sort -i - | bgzip -c /dev/stdin > {output.bed} ) &> {log}
        tabix -f {params.tabixParams} -p bed {output.bed} && touch {output.index}
        """

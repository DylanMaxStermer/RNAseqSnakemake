...
rule annotate_juncfiles:
    ...
    input:
        ...
        juncs = "results/SplicingAnalysis/juncfiles/{sample}.junc",
    output:
        counts = "results/SplicingAnalysis/juncfiles/{sample}.junccounts.tsv.gz"
    log:
...
rule leafcutter_cluster:
    ...
    output:
        outdir = directory("results/SplicingAnalysis/leafcutter/{GenomeName}/clustering/"),
        counts = "results/SplicingAnalysis/leafcutter/{GenomeName}/clustering/leafcutter_perind.counts.gz",
        numers = "results/SplicingAnalysis/leafcutter/{GenomeName}/clustering/leafcutter_perind_numers.counts.gz"
    shadow: "shallow"
    resources:
        mem_mb = GetMemForSuccessiveAttempts(24000, 48000)
    log:
    ...
    shell:
        """
        python workflow/scripts/leafcutter/clustering/leafcutter_cluster_regtools.py -j {input.juncfile_list} {params} -r {output.outdir} -k True &> {log}
        """
...
rule leafcutter_to_PSI:
    input:
        numers = "results/SplicingAnalysis/leafcutter/{GenomeName}/clustering/leafcutter_perind_numers.counts.gz"
    output:
        juncs = temp("results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/JuncCounts.bed"),
        PSI = temp("results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/PSI.bed"),
        PSIByMax = temp("results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/PSI_ByMax.bed"),
        PSIDenom = temp("results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/PSIDenom.bed"),
        PSIByMaxDenom = temp("results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/PSI_ByMaxDenom.bed")
    log:
    ...
    shell:
        """
        Rscript  workflow/scripts/leafcutter_to_PSI.R {input.numers} {output.PSI} {output.PSIByMax} {output.juncs} {output.PSIDenom} {output.PSIByMaxDenom} &> {log}
        """

rule bgzip_PSI_bed:
    input:
        bed = "results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/{Metric}.bed",
    output:
        bed = "results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/{Metric}.sorted.bed.gz",
        index  = touch("results/SplicingAnalysis/leafcutter/{GenomeName}/juncTableBeds/{Metric}.sorted.bed.gz.indexing_done"),
    log:
...
rule Get5ssSeqs:
    ...
    """
    input:
        basic = "results/SplicingAnalysis/ObservedJuncsAnnotations/{GenomeName}.uniq.annotated.tsv.gz",
        fa = config['GenomesPrefix'] + "{GenomeName}/Reference.fa",
    output:
        "results/SplicingAnalysis/ObservedJuncsAnnotations/{GenomeName}.uniq.annotated.DonorSeq.tsv"
    shell:
        """
...
rule ConvertLeacfutterJuncNames_To_True_Bed_Coords:
    ...
    output:
        "results/SplicingAnalysis/ClassifyJuncs/{GenomeName}.juncList.tsv.gz"
    shell:
        """
...
rule leafcutter2_ClassifyJuncs_ClusterPerInd:
    ...
    """
    input:
        junclist = "results/SplicingAnalysis/ClassifyJuncs/{GenomeName}.juncList.tsv.gz",

        gtf = config['GenomesPrefix'] + "{GenomeName}/Reference.gtf",
        fa = config['GenomesPrefix'] + "{GenomeName}/Reference.fa",
        fai = config['GenomesPrefix'] + "{GenomeName}/Reference.fa.fai",
    output:
        outdir = directory("results/SplicingAnalysis/ClassifyJuncs/{GenomeName}"),
...
rule leafcutter_ds_contrasts:
    input:
        ...
        numers = lambda wildcards: get_filled_path_from_contrast(wildcards, "results/SplicingAnalysis/leafcutter/{GenomeName}/clustering/leafcutter_perind_numers.counts.gz"),
    output:
...
rule tidy_leafcutter_differentialSplicing:
    ...
    input:
        ...
        Classifications = lambda wildcards: get_filled_path_from_contrast(wildcards, "results/SplicingAnalysis/ClassifyJuncs/{GenomeName}/Leaf2_junction_classifications.txt"),
        effect_sizes = "results/SplicingAnalysis/differential_splicing/{contrast}/leaf_effect_sizes.txt",
        pvalues = "results/SplicingAnalysis/differential_splicing/{contrast}/leaf_cluster_significance.txt",
    output:

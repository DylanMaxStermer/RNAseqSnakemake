rule Make_new_samples_config:
    output:
        samples_tsv = "results/samples.SRA_accession_links_filled.tsv"
    run:
...
rule CopyFastq:
    ...
    output:
        R1 = temp("results/Fastq/{sample}.R1.fastq.gz"),
        R2 = temp("results/Fastq/{sample}.R2.fastq.gz"),
...
rule DownloadFromAccession:
    ...
    output:
        fastq = temp("results/Fastq/{sample}.{Read}.fastq.gz"),
    log:
...
rule fastp:
    input:
        R1 = "results/Fastq/{sample}.R1.fastq.gz",
        R2 = "results/Fastq/{sample}.R2.fastq.gz",
    output:
        R1 = temp("results/FastqFastp/{sample}.R1.fastq.gz"),
        R2 = temp("results/FastqFastp/{sample}.R2.fastq.gz"),
        html = "results/FastqFastp/{sample}.fastp.html",
        json = "results/FastqFastp/{sample}.fastp.json"
    params:
...
rule STAR_Align:
    input:
        ...
        R1 = "results/FastqFastp/{sample}.R1.fastq.gz",
        R2 = lambda wildcards: "results/FastqFastp/{sample}.R2.fastq.gz" if wildcards.sample in samples_PairedEnd.tolist() else []
    output:
        outdir = directory("results/Alignments/{sample}"),
        bam = "results/Alignments/{sample}/Aligned.sortedByCoord.out.bam",
        align_log = "results/Alignments/{sample}/Log.final.out"
    threads: 8
...
rule indexBam:
    ...
    output:
        index = touch("results/Alignments/{sample}/Aligned.sortedByCoord.out.bam.indexing_done"),
...
rule idxstats:
    ...
    output:
        "results/idxstats/{sample}.idxstats.txt"
    shell:
        """
